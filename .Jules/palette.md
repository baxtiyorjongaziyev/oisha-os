...
## 2025-08-01 - Global keyboard shortcut form focus exclusion\n**Learning:** The global '/' shortcut to open search incorrectly assumed 'INPUT' and 'TEXTAREA' check was sufficient. It was hijacking focus from rich text editors (contenteditable) and select fields.\n**Action:** Always check `e.target.isContentEditable` and expand tag checks to `SELECT` when implementing global keystroke capture to prevent form input hijack.
