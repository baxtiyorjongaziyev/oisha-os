...
\n## 2026-06-10 - [A11y Insight] Stable DOM structures for collapsible UI elements\n\n**Learning:** When hiding text labels in collapsible UI elements (like a sidebar) to leave only icons visible, conditionally removing the text node from the DOM entirely breaks the context for screen readers.\n**Action:** Always maintain a stable DOM structure by using utility classes like `sr-only` to visually hide the text when collapsed, ensuring screen readers still announce the labels for icon-only interactive elements.
