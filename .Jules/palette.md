...
## 2024-05-15 - [A11y Improvement] Semantic Lists for Grid Items

**Learning:** Rendering content items (like capabilities or feature lists) as independent `<div>` elements inside a generic `<section>` grid prevents screen readers from understanding that the items belong to a grouped collection, missing out on contextual announcements like "List with 4 items".
**Action:** Always wrap grouped items, particularly those shown in a visual list or grid pattern, in a semantic `<ul>` (with an appropriate `aria-label`) and use `<li>` for the individual items. This ensures proper structural semantics and screen reader context.
