...
from src.handlers import negotiation, shadow_advisor
...
def test_private_userbot_reply_policy_defaults_to_block(monkeypatch):
    ...
    assert negotiation._should_block_private_userbot_reply(event) is True
...
def test_private_userbot_reply_policy_allows_groups(monkeypatch):
    ...
    assert negotiation._should_block_private_userbot_reply(event) is False
...
async def test_negotiation_handler_ignores_private_dm_by_policy(monkeypatch):
    ...
    monkeypatch.setattr(app_ctx, "auto_lead_agent", SimpleNamespace(extract_lead_info=AsyncMock()))

    await negotiation.negotiation_agent_handler(event)

    app_ctx.auto_lead_agent.extract_lead_info.assert_not_awaited()
...
async def test_shadow_advisor_ignores_private_dm_by_policy(monkeypatch):
    ...
    event = SimpleNamespace(
        ...
        chat_id=77,
        message=SimpleNamespace(text="salom"),
        get_sender=AsyncMock(return_value=SimpleNamespace(bot=False, first_name="User")),
    )

    await shadow_advisor.shadow_advisor_handler(event)
