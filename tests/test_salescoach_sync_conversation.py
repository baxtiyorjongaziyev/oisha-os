...
def test_service_token_uses_secret_value(monkeypatch):
    monkeypatch.setattr(settings, "SALESCOACH_API_URL", "https://salescoach.test")
    monkeypatch.setattr(settings, "SALESCOACH_SERVICE_TOKEN", SecretStr("real-token"))
    monkeypatch.setattr(settings, "SALESCOACH_ENABLED", True)

    sync = SalesCoachSync()

    assert sync.token == "real-token"
    assert sync.token != "**********"


@pytest.mark.asyncio
async def test_messages_are_filtered_and_bounded():
    sync = SalesCoachSync()
    sync.enabled = True
    fake = FakeAsyncClient(payload={"overallScore": 78})
    sync._client = fake

    await sync.analyze_conversation(
        lead_id=42,
        manager_id="77",
        messages=[
            {"id": 1, "role": "customer", "text": "   "},
            {"id": 2, "role": "manager", "text": "x" * 5000},
        ],
    )

    assert len(fake.last_json["messages"]) == 1
    assert len(fake.last_json["messages"][0]["text"]) == 4000
