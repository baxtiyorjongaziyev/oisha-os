...
async def test_low_confidence_analysis_never_auto_writes(dependencies):
    coach, store, sync, _, writer, _, _, notifier = dependencies
    coach.mode = "auto"
    ...
    assert store.save_analysis.await_args.args[0].status == "pending"


@pytest.mark.asyncio
async def test_medium_crm_confidence_routes_to_approval(dependencies):
    coach, store, _, matcher, writer, _, _, notifier = dependencies
    coach.mode = "auto"
    matcher.match.return_value = CrmMatch(
        lead_id=42,
        contact_id=84,
        responsible_user_id=777,
        confidence=0.84,
