...
import { getCrmDashboardStats, getFrogTasks, FrogTask } from '@/lib/apiClient';
...
export default async function TasksDashboard() {
  ...
  const pendingCount = stats?.tasks?.pending ?? tasks.filter(t => t.status !== 'Done' && t.status !== 'Completed').length;
  const doneCount = stats?.tasks?.completed_today ?? tasks.filter(t => t.status === 'Done' || t.status === 'Completed').length;
  const overdueCount = stats?.tasks?.overdue ?? 0;
  const frogTask = tasks.find(t => t.is_frog) || tasks[0];
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[var(--text-secondary)]">• 100% Real Turso DB Vazifalari</span>
          </div>
          <h1 className="mt-1 text-2xl md:text-3xl font-bold tracking-tight text-[var(--text-primary)]">
            Vazifalar & Jamoa Yuklamasi
          </h1>
          <p className="mt-0.5 text-xs text-[var(--text-secondary)]">
            Har kuni 09:00 da Telegram orqali eng yuqori daromad keltiruvchi vazifalar (Eat The Frog) saralanadi.
          </p>
        </div>
      ...
      {/* Google Keep / Linear Frog Hero Banner (if frogTask exists) */}
      {frogTask ? (
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <span className="text-[9px] font-extrabold uppercase tracking-wider text-[#7c3aed] dark:text-[#c084fc]">
                  Kunning 1-raqamli Vazifasi (Eat The Frog)
                </span>
                <h3 className="text-sm md:text-base font-bold text-[var(--text-primary)] mt-0.5">
                  {frogTask.title}
                </h3>
                <p className="text-xs text-[var(--text-secondary)] mt-0.5">
                  Kutilgan daromad: <strong className="text-[#059669] dark:text-[#34d399]">+${(frogTask.profit_estimate || 0).toLocaleString()}</strong> • Mas&apos;ul: <strong>{frogTask.assigned_to || 'Jamoa'}</strong> • Muddat: <strong>{frogTask.deadline || 'Bugun'}</strong>
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 shadow-xs">
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-[var(--text-secondary)] border-t border-[var(--border-subtle)] pt-2.5">
            <span>Holat:</span>
            <strong className="text-[var(--text-primary)]">Faol jarayonda</strong>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] p-5 shadow-xs">
          ...
          <div className="mt-3 flex items-center justify-between text-xs text-[var(--text-secondary)] border-t border-[var(--border-subtle)] pt-2.5">
            ...
            <strong className="text-[#7c3aed] dark:text-[#c084fc]">{doneCount} ta</strong>
          </div>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--border-default)] bg-[var(--bg-surface)] shadow-xs overflow-hidden">
        <div className="border-b border-[var(--border-default)] px-5 py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-[var(--text-secondary)]">Turso libSQL bazasidagi real topshiriqlar ({tasks.length} ta)</p>
          </div>
        </div>
        ...
        {tasks.length === 0 ? (
          <div className="py-14 text-center">
            ...
            <h3 className="font-bold text-sm text-[var(--text-primary)]">Hozircha vazifalar mavjud emas</h3>
            <p className="text-xs text-[var(--text-secondary)] mt-1 max-w-sm mx-auto">
              Telegram orqali yoki FrogAgent yangi topshiriq yaratganda bu yerda real vaqtda ko&apos;rinadi.
            </p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-[var(--border-subtle)]">
                {tasks.map((task: FrogTask) => (
                  <tr key={task.id} className="hover:bg-[var(--bg-surface-subtle)] transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2 font-bold text-[var(--text-primary)]">
                        {task.is_frog && <span className="text-sm">🐸</span>}
                        <span>{task.title}</span>
                      </div>
                      {task.description && (
                        <div className="text-[11px] text-[var(--text-secondary)] mt-0.5 line-clamp-1">{task.description}</div>
                      )}
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        <div className="h-6 w-6 rounded-full bg-[#ede9fe] text-[#5b21b6] dark:bg-[#4c1d95] dark:text-[#c084fc] flex items-center justify-center text-[10px] font-bold">
                          {String(task.assigned_to || 'J').charAt(0)}
                        </div>
                        <span className="font-semibold text-[var(--text-primary)]">{String(task.assigned_to || 'Jamoa')}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2">
                        ...
                        {task.profit_estimate ? (
                          <span className="font-bold text-[#059669] dark:text-[#34d399]">+${task.profit_estimate.toLocaleString()}</span>
                        ) : null}
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-[var(--text-secondary)] font-mono">
                      {task.deadline || 'Belgilanmagan'}
                    </td>
