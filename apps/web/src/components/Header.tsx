...
export default function Header() {
  ...
  const handleBugSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!bugText.trim() || isBugSubmitting) return;
    setBugSuccess(false);
    setBugError("");
            ...
            <form onSubmit={handleBugSubmit} className="mt-4 space-y-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  aria-disabled={isBugSubmitting}
                  aria-busy={isBugSubmitting}
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-brand px-4 py-2 text-xs font-medium text-white hover:bg-brand-hover shadow-md transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isBugSubmitting && (
                    <svg
                      aria-hidden="true"
                  ...
                  {isBugSubmitting ? "Yuborilmoqda..." : "Yuborish"}
                </button>
              </div>
            </form>
          </div>
