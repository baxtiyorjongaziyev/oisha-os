...
### 2026-09-10 — claude — Call-analiz topic routing fix + obsolete plan removal
- **Ish (PR #617, merged d2f136f2):** AI qo'ng'iroq tahlili xulosasi sotuv o'qitish topic'iga hech qachon tushmayotgan edi. Sabab: `call_notifier.py` + `admin_bot/alerts.py notify_team` topic id'ni `reply_to_message_id` sifatida uzatardi — forum topic uchun ishlamaydi (Telethon "General"ga tashlaydi). `message_thread_id` ga o'tkazildi; `TelethonBotRuntime` endi `InputReplyToMessage(top_msg_id=...)` yasaydi va dict-uslub tugmalarni Telethon `Button` ga o'giradi. Call xulosalari uchun alohida `CALL_ANALYSIS_GROUP_ID` / `CALL_ANALYSIS_TOPIC_ID` (548), unset bo'lsa `AMOCRM_ALERT_FORWARD_*` (443) ga qaytadi. Follow-up deadline eslatmalari (`AmoCrmTaskNotifier`) allaqachon `agreed_datetime`ni `complete_till` qiladi — forum bug fix bilan endi topic 443 ga yetadi.
- **O'zgargan fayllar:** src/settings.py (CALL_ANALYSIS_GROUP_ID/TOPIC_ID), src/services/core/calls/call_notifier.py, src/services/core/telegram/bot_runtime.py (`_coerce_telethon_buttons`, forum `InputReplyToMessage`), src/services/core/admin_bot/alerts.py (notify_team → message_thread_id), .env.example, tests: test_call_notifier_topic.py (new), test_bot_runtime.py, test_admin_bot_notify.py, test_call_analyzer_telegram_alert.py, test_command_center_scheduler.py.
- **Tekshiruv:** local `SKIP_LIVE=1 pytest -q` 2001 passed/18 skipped; CI pull-request-tests green; bandit clean. Oracle deploy `42b7fff5` success (mening d2f136f2 ustiga qurilgan).
- **Reja o'chirildi:** `docs/superpowers/plans/2026-09-09-aiogram-telethon-hybrid.md` (1206 qator, AGENTS.md 400-qator limitini buzardi) — reja ortiqcha edi: aiogram/telethon hybrid allaqachon prodda (PR #367 + 2026-09-07 wiring restore). 42 aiogram testi yashil. Standalone docs PR (#620) required test CI path-filter tufayli merge bo'lmadi; o'chirilish `fix/split-aiogram-plan` branch'da, keyingi src/ PR bilan olib ketiladi.
- **Qolgan ish:** Oracle `.env` ga 4 kalit (443 + 548); @jonairobot 548/443 topic yozish huquqi; live smoke — test qo'ng'iroq → topic 548. `fix/split-aiogram-plan` branch merge (src PR bilan).
