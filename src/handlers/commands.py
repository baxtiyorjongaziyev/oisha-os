...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db, crm=None) -> None:
...
async def register_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db) -> None:
...
async def branding_command(update: Update, context: ContextTypes.DEFAULT_TYPE, ai_client, types) -> None:
    ...
    try:
        ...
        if response.generated_images:
            ...
            await update.message.reply_photo(photo=image_bytes, caption=f"🎨 Tavsif: _{prompt}_", parse_mode="HTML")
...
async def task_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db, ai_client=None) -> None:
    ...
    if not context.args:
        await update.message.reply_text("❌ `/task [vazifa tafsiloti]`", parse_mode="Markdown")
        return
    ...
    if ai_client:
        try:
            ...
            who, task_desc, deadline = data.get("who"), data.get("task"), data.get("deadline")
            task_id = db.add_task(who, task_desc, deadline)
            await update.message.reply_text(f"📌 **YANGI VAZIFA [#{task_id}]**\nMas'ul: {who}\nTask: {task_desc}")
            return
        except Exception as e:
...
async def portfolio_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Send portfolio link."""
    keyboard = [[InlineKeyboardButton("Portfelni ochish 🎨", web_app=WebAppInfo(url=config.WEBAPP_URL))]]
    await update.message.reply_text("Mening ishlarim bilan tanishing:", reply_markup=InlineKeyboardMarkup(keyboard))

async def broadcast_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db) -> None:
...
async def unregister_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db) -> None:
    """Remove user from team."""
    if update.effective_user.id != config.OWNER_ID: return
    if not context.args: return
    target_id = int(context.args[0])
    if db.set_user_role(target_id, "user"):
        await update.message.reply_text(f"✅ Foydalanuvchi {target_id} jamoadan chiqarildi.")

async def amofix_command(update: Update, context: ContextTypes.DEFAULT_TYPE, amocrm) -> None:
...
async def pay_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await context.bot.send_invoice(
        update.message.chat_id, "Xizmat to'lovi", "Tavsif", "bot-service-payment",
        config.PROVIDER_TOKEN, "UZS", prices
    )

async def analyze_history_command(update: Update, context: ContextTypes.DEFAULT_TYPE, scouter, db, learner) -> None:
    """Analyze history from 2025 onwards (Owner only)."""
    if str(update.effective_user.id) != str(config.OWNER_ID): return
    await update.message.reply_text("🕵️‍♂️ 2025-yildan beri bo'lgan barcha shaxsiy suhbatlarni tahlil qilish boshlandi...")
