    ...
    for i in range(retries):
        try:
            ...
            if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                ...
                logger.warning(f"[GLOBAL AI] Rate limit hit (429). Retrying in {wait_time:.2f}s... ({i+1}/{retries})")
                await asyncio.sleep(wait_time)
            elif "500" in err_str or "503" in err_str:
                wait_time = (i + 1) * 2
                logger.warning(f"[GLOBAL AI] Server error ({err_str[:3]}). Retrying in {wait_time}s...")
                await asyncio.sleep(wait_time)
            else:
                raise e
    return None
