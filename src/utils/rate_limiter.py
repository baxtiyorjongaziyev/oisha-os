...
class AdaptiveRateLimiter:
    ...
    def __init__(
        ...
        self._limiter = TokenBucketRateLimiter(
            RateLimitConfig(
                requests_per_second=initial_rps, burst_size=max(1, int(initial_rps * 2))
            )
        )
        ...
        async with self._lock:
            ...
            if self.success_count >= 10:
                self.current_rps = min(
                    self.max_rps, self.current_rps * self.recovery_factor
                )
        ...
        async with self._lock:
            ...
            if is_rate_limit:
                ...
                self.current_rps = max(
                    self.min_rps, self.current_rps * self.backoff_factor
                )
                self._update_limiter()
                self.success_count = 0
...
class TelegramRateLimiter:
    ...
    def __init__(self):
        ...
        self._adaptive = AdaptiveRateLimiter(initial_rps=1.0, min_rps=0.1, max_rps=5.0)

    async def can_send_message(self, chat_id: int, is_group: bool = False) -> bool:
        """
    ...
    async def wait_to_send(
        self, chat_id: int, is_group: bool = False, timeout: float = 60.0
    ) -> bool:
        """
    ...
    def report_flood_wait(self, retry_after: int):
        ...
        if self._flood_wait_until is None or wait_until > self._flood_wait_until:
            ...
            logger.warning(f"[TELEGRAM RATE LIMIT] Flood wait: {retry_after}s")

        # Also notify adaptive limiter
        asyncio.create_task(self._adaptive.report_error(is_rate_limit=True))

    async def execute_with_limit(self, chat_id: int, is_group: bool, coro):
        """
