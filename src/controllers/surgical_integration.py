...
from src.agents import get_surgical_negotiator, negotiate
...
class SurgicalIntegration:
    ...
    async def process_message(
        ...
        try:
            ...
            requires_human = (
                result.get("requires_approval", False) or
                result['assessment'].get('close_probability', 0) < 0.2 or
                result['assessment'].get('objection') == "legal"
            )
    ...
    def should_use_surgical(self, user_id: str, message: str) -> bool:
        ...
        sales_keywords = [
            "narx", "narxi", "price", "budget", "budjet",
            "xizmat", "service", "ish", "loyiha", "project",
            "shartnoma", "contract", "agreement",
            "taklif", "offer", "proposal",
            "buyurtma", "order",
            "savdo", "sale",
            "qancha", "how much", "cost",
            "web", "sayt", "site", "branding", "design",
            "marketing", "reklama", "advertising",
            "kerak", "need", "want", "xohlayman"
        ]
        ...
        intent_markers = [
            "?",  # Questions
            "qilasizmi", "bormi", "mumkinmi",  # Inquiries
        ]
...
async def test_integration():
    ...
    for user_id, message in test_cases:
        ...
        if should_use:
            ...
            print(f"Probability: {result.get('deal_info', {}).get('probability', 0):.0%}")
