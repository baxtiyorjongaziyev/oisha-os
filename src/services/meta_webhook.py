...
def get_ai_reply(user_id: str, user_name: str, user_message: str, platform: str = "Instagram") -> str:
    ...
    try:
        ...
        manager.register_agent(SalesAgent("sales", settings.SALES_PROMPT, settings.AI_KEYS))
        manager.register_agent(SupportAgent("support", settings.SUPPORT_PROMPT, settings.AI_KEYS))
        ...
        reply = orchestrator.get_agent_response(intent, int(user_id) if str(user_id).isdigit() else 0, enhanced_message)
...
def notify_crm(source: str, user_name: str, user_id: str, message: str, reply: str):
    ...
    quality = "Unknown"
    if "[LEAD_REPORT: QUALITY=sifatli]" in reply.lower(): quality = "Sifatli 💎"
    elif "[LEAD_REPORT: QUALITY=oddiy]" in reply.lower(): quality = "Oddiy ✅"
        ...
        for entry in data.get("entry", []):
            ...
            if messaging:
                ...
                if text:
                    ...
                    # AI javob
                    ai_reply = get_ai_reply(str(sender_id), "Foydalanuvchi", str(text), platform="Instagram DM")
                    
                    # Ma'lumotlarni saqlash (SAVE_INFO tag)
                    import re
                    save_matches = re.finditer(r'\[SAVE_INFO:\s*(.*?)\]', str(ai_reply), re.IGNORECASE)
                    info_updates = {}
            ...
            for change in entry.get("changes", []):
                ...
                if comment_text and comment_id:
                    logger.info(f"[META COMMENT] {commenter_name}: {str(comment_text)[:50]}")
