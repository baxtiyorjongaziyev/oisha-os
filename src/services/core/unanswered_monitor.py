...
class UnansweredMonitor:
    def __init__(self, amocrm: AmoCRMSync, bot_client, db: Database, auto_lead: AutoLeadAgent):
        self.amocrm = amocrm
    ...
    async def start_monitoring(self, interval_seconds: int = 1800):
        ...
        logger.info(f"👸 [MONITOR] Unanswered messages monitor started with interval {interval_seconds}s")
        while True:
            try:
    ...
    async def check_and_notify(self):
        ...
        leads = await self.amocrm.get_leads(filter_status=[142, 143]) # Example statuses for New/Incoming
        if not leads:
            return
        ...
        for lead in leads:
            ...
            if (now - last_msg_time) > 1200:
                ...
                buttons = [
                    ...
                    [Button.url("🌐 amoCRM-da ochish", f"https://{self.amocrm.subdomain}.amocrm.ru/leads/detail/{lead_id}")]
                ]
                
                try:
                    await self.bot_client.send_message(target_chat, msg, buttons=buttons, parse_mode='markdown')
    ...
    async def send_draft_to_crm(self, lead_id: int, text: str):
        ...
        return await self.amocrm.add_lead_note(lead_id, f"👸 Oisha (Menejer nomidan): {text}")
