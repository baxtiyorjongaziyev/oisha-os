...
class ChannelForwarder:
    ...
    def __init__(
        self, client: TelegramClient, db, monitor_channel: str, juma_notifier=None
    ):
        self.client = client
        self.db = db
    ...
    async def start(self):
        ...
        logger.info(
            f"👸 [FORWARDER] Smart monitoring active for: {self.monitor_channel}"
        )
        ...
        async def handler(event):
            ...
            if now.weekday() == 4 and self.juma_notifier:
                ...
                if is_juma:
                    logger.info(
                        "👸 [FORWARDER] Juma greeting detected! Forwarding to all classmates..."
                    )
                    asyncio.create_task(
                        self.juma_notifier.send_greetings(
                            now.strftime("%Y-%m-%d"), source_message=event.message
                        )
                    )
                    return
            ...
            if "#forward" in text.lower():
                logger.info(
                    "👸 [FORWARDER] Trigger #forward detected! Forwarding to business groups..."
                )
    ...
    async def forward_to_business_groups(self, message):
        ...
        try:
            ...
            if not targets:
                logger.warning(
                    "👸 [FORWARDER] No business groups found for forwarding."
                )
                return

            logger.info(
                f"👸 [FORWARDER] Forwarding to {len(targets)} business groups..."
            )
