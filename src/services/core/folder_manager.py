...
class FolderManager:
    ...
    async def _fetch_filters(self):
        ...
        try:
            self.existing_filters = await self.client(
        ...
        try:
            ...
            if target_filter:
                ...
                if user_id not in peer_ids:
                    ...
                    await self.client(
                        functions.messages.UpdateDialogFilterRequest(
                            id=target_filter.id, filter=target_filter
                        )
                    )
                    logger.info(
                        f"[FOLDER] User {user_id} added to existing folder: {folder_title}"
                    )
            else:
                # Create new folder
                ...
                if new_id > 25:  # Safety break, though Premium handles many
                    logger.warning(
                        f"[FOLDER] Too many folders already (ID {new_id}). Skipping."
                    )
                    return False
                ...
                await self.client(
                    functions.messages.UpdateDialogFilterRequest(
                        id=new_id, filter=new_filter
                    )
                )
                logger.info(
                    f"[FOLDER] New folder created and user {user_id} added: {folder_title}"
                )
