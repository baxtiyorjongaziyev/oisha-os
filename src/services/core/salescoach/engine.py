"""
TelegramSalesCoach orchestrator for monitoring private dialogues and triggering analyses.
"""
...
class TelegramSalesCoach:
        ...
        *,
        ...
        if self._self_id is None:
            ...
            try:
                ...
                resolved = int(getattr(me, "id", 0) or 0)
                if resolved <= 0:
                    return
                self._self_id = resolved
            except Exception:
                return
        if self._self_id == telegram_user_id:
            return
    ...
    async def analyze_dialog_now(
        ...
        if not isinstance(analysis, Mapping):
            recorder = getattr(self.store, "record_analysis_failure", None)
            ...
            return None
        clearer = getattr(self.store, "clear_analysis_failure", None)
        if callable(clearer):
            await _maybe_await(clearer(fingerprint))
        ...
        try:
            overall_score = max(
                0,
                min(
                    int(analysis.get("overallScore") or analysis.get("overall_score") or 0),
                    100,
                ),
            )
        except (TypeError, ValueError):
        ...
        approval_required = (
            self.mode == "approval"
            or (self.mode == "auto" and match.confidence < 0.85)
            or (self.mode == "auto" and analysis_confidence < 0.80)
        )
        analysis_id = await self.store.save_analysis(
            ConversationAnalysisRecord(
                ...
                status="pending" if approval_required else "analyzed",
            )
        )
