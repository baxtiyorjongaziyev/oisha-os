...
class SafeResponder:
    ...
    def __init__(self, allowed_groups: list = None):
        ...
        self._global_response_times: deque = deque(
            maxlen=max(GLOBAL_HOURLY_LIMIT * 2, 60)
        )
    ...
    async def should_respond(self, event, my_id: int = None) -> bool:
        ...
        is_team_member = (
            sender_id in self.team_whitelist or sender_username in self.team_whitelist
        )

        if not is_team_member:
            # Mijoz yoki notanish odam bo'lsa, javob bermaymiz
            logger.debug(
                f"[SAFE] Blocking non-team member: {sender_id} ({sender_username})"
            )
            return False
    ...
    async def prepare_to_reply(self, event, client):
        ...
        async with client.action(event.chat_id, "typing"):
            ...
            logger.info(
                f"[SAFE] Delaying reply for {delay:.2f}s to chat {event.chat_id}"
            )
    ...
    def is_rate_limited(self, chat_id) -> tuple[bool, str]:
        ...
        if len(self._global_response_times) >= GLOBAL_HOURLY_LIMIT:
            return (
                True,
                f"global_hourly({len(self._global_response_times)}/{GLOBAL_HOURLY_LIMIT})",
            )
