...
class HistoricalSyncService:
    def __init__(self, db: Database, client: TelegramClient):
        self.db = db
        self.client = client
        self.auto_lead_agent = AutoLeadAgent(
    ...
    async def start_backlog_sync(self, days: int = 365, limit_dialogs: int = 500):
        ...
        try:
            ...
            async for dialog in self.client.iter_dialogs(limit=limit_dialogs):
                ...
                logger.info(
                    f"🔍 [HISTORICAL] Syncing chat: {dialog.name} ({dialog.id})"
                )
            ...
            logger.info(
                f"✅ [HISTORICAL] Sync finished. Processed {dialog_count} dialogs."
            )
            ...
            async for msg in self.client.iter_messages(dialog.id, offset_date=None):
                ...
                if msg.text:
                    ...
                    messages_to_log.append(
                        (dialog.id, text, msg.out, msg.date.isoformat())
                    )
            ...
            if full_chat_text:
                ...
                asyncio.create_task(
                    run_autonomous_advice(
                        dialog.id, dialog.name, "HISTORY_SYNC_TRIGGER"
                    )
                )

        except errors.FloodWaitError as e:
            logger.warning(
                f"⏳ [FLOOD] Waiting for {e.seconds} seconds due to Telegram limits."
            )
