...
class EscalationAgent:
    ...
    async def check_pending_feedbacks(self):
        ...
        for action in actions:
            ...
            if datetime.datetime.now() - created_at > datetime.timedelta(hours=self.escalation_threshold_hours):
                ...
                if data.get('escalated'): continue
                ...
                async with conn.execute(query, (target_user_id, action['created_at'])) as cursor:
                ...
                if reply_count == 0:
                    ...
                    logger.info(f"[ESCALATION] User {target_user_id} replied to alert. No escalation needed.")
                    # Allaqachon ko'rilgan deb belgi qo'yamiz (logda)
                    data['escalated'] = False
                    data['replied'] = True
                    await self.db.log_agent_action(action['user_id'], "alert_replied", data)
    ...
    async def _escalate(self, action: Dict[str, Any]):
        ...
        logger.warning(f"[ESCALATION] Alert not replied by {user_id}. Escalating to Admin...")
        
        owner_id = int(config.OWNER_ID or 0)
        if not owner_id or not self.bot_client:
            logger.error("[ESCALATION] Owner ID or Bot Client missing. Cannot escalate.")
            return

        user_info = await self.db.get_user_info(user_id)
        name = user_info['first_name'] if user_info else f"ID: {user_id}"
        username = f"@{user_info['username']}" if user_info and user_info['username'] else name
