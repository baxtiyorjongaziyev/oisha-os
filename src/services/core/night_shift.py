...
class NightShiftService:
    ...
    def __init__(self, bot_client=None):
        ...
        self.researcher = ResearcherAgent(
            "night_researcher", config.SYSTEM_INSTRUCTION, self.api_keys
        )
        ...
        while True:
            try:
                ...
                if slot_ok(0, 0) and self._should_run_once_today("history_sync", now):
                    ...
                    add_activity(
                        "🚀 Historical Sync boshlandi",
                        "1 yillik yozishmalar tahlil qilinmoqda...",
                        "info",
                    )
                    if self.bot:
                ...
                if slot_ok(1, 0) and self._should_run_once_today("hygiene", now):
                    ...
                    add_activity(
                        "🌙 CRM Hygiene boshlandi",
                        "Airtable va AmoCRM ma'lumotlari bir xillashtirilmoqda...",
                        "info",
                    )
                ...
                if slot_ok(2, 0) and self._should_run_once_today("lead_intel", now):
                    ...
                    add_activity(
                        "🔍 Lead Intelligence boshlandi",
                        "Yangi lidlar bo'yicha OSINT tadqiqot o'tkazilmoqda...",
                        "info",
                    )
                ...
                if slot_ok(4, 0) and self._should_run_once_today("battlecards", now):
                    ...
                    add_activity(
                        "🚀 Battlecards tayyorlanmoqda",
                        "Bugungi uchrashuvlar tahlili boshlandi...",
                        "info",
                    )
    ...
    async def perform_lead_research(self):
        ...
        try:
            ...
            leads = await self.crm.get_leads(
                status_id=(
                    config.AMOCRM_INITIAL_STATUS_ID
                    if hasattr(config, "AMOCRM_INITIAL_STATUS_ID")
                    else None
                )
            )
            ...
            for lead in leads[:10]:  # Limit for stability
                ...
                # Write back to CRM
                await self.crm.add_lead_note(
                    lead_id, f"🔍 OISHA LEAD INTELLIGENCE:\n\n{analysis}"
                )
                from src.api_server import add_activity

                add_activity(
                    "Lid boyitildi",
                    f"{lead_name} bo'yicha OSINT ma'lumotlar AmoCRM'ga yozildi.",
                    "success",
                )
    ...
    async def generate_daily_reflection(self):
        ...
        try:
            ...
            for event in events:
                ...
                brief = await self.researcher.process_task(
                    0,
                    f"Uchrashuv: {summary}. Tavsif: {description}. Ushbu uchrashuvda qanday strategik yondashuv kerakligini 1 gapda ayting.",
                )
                report += (
                    f"⏰ **{start[11:16]}** — {summary}\n💡 *Maslahat:* {brief}\n\n"
                )
            ...
            if self.bot:
                ...
                await self.bot.send_message(
                    settings.OWNER_ID, report, parse_mode="markdown"
                )
