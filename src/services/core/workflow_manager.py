...
class WorkflowManager:
    def __init__(self, db: Database, crm: AmoCRMSync, client=None):
        ...
        self.crm = crm
        self.client = client
        self.reporter = EnterpriseReporter(db, crm) # Lazy init or passing actual CRMService is better
    ...
    async def notify_assignment(self, tg_user_id: int, lead: dict):
        ...
        amocrm_link = f"https://{settings.AMOCRM_SUBDOMAIN}.amocrm.ru/leads/detail/{lead_id}"
        ...
        try:
            ...
            logger.info(f"[WORKFLOW] Notification sent to {tg_user_id} for lead {lead_id}")
    ...
    async def check_airtable_reminders(self):
        ...
        for record in records:
            ...
            project_name = fields.get("Project Name") or fields.get("Loyiha nomi") or "Unknown"
            ...
            if stage.lower() not in ["done", "yakunlandi", "arxiv"]:
                ...
                try:
                    ...
                    logger.info(f"[WORKFLOW] Direct PM reminder sent for: {project_name}")
                except Exception as e:
