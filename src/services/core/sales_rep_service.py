...
class SalesRepService:
    ...
    def get_stagnated_leads(self, leads: list[dict]) -> list[dict]:
        ...
        for lead in leads:
            ...
            threshold = STAGNATION_HOURS.get(status_name, DEFAULT_STAGNATION_HOURS)
            ...
            if updated_at:
                ...
                if hours_stale > threshold:
                    lead["_stagnation_hours"] = int(hours_stale)
    ...
    async def send_daily_digest(self, bot, manager_tg_id: int, manager_amo_id: int) -> None:
        try:
            ...
            text = (
                "\U0001f3af *Bugungi TOP-5 lidingiz*\n\n"
                + "\n\n".join(lines)
                + f"\n\n\U0001f4ca Jami ochiq: {len(leads)} ta lid"
            )
            await bot.send_message(
                manager_tg_id, text, parse_mode="markdown", disable_web_page_preview=True
            )
