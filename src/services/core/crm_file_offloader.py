...
class CRMFileOffloader:
    ...
    async def _ensure_folder(self) -> str:
        ...
        logger.info(
            "[OFFLOADER] GDRIVE_OFFLOAD_FOLDER_ID topilmadi. Yangi yaratilmoqda..."
        )
                ...
                if note.get("note_type") == "file":
                    ...
                    try:
                        ...
                        try:
                            ...
                            drive_link = self.gdrive.upload_file(
                                tmp_path, folder_id=folder_id
                            )
                            ...
                            if drive_link:
                                ...
                                success = await self.amo.delete_note(
                                    "leads", lead_id, note["id"]
                                )
                                if success:
                                    logger.info(
                                        f"   ✅ {file_name} muvaffaqiyatli ko'chirildi va CRM-dan o'chirildi."
                                    )
                                    stats["offloaded"] += 1
                                else:
                                    logger.warning(
                                        f"   ⚠️ {file_name} ko'chirildi, lekin originalni o'chirib bo'lmadi."
                                    )
                            else:
                                logger.error(
                                    f"   ❌ Drive-ga yuklashda xato: {file_name}"
                                )
                                stats["errors"] += 1
                        finally:
                        ...
                        logger.error(
                            f"   ❌ Lead {lead_id} faylini qayta ishlashda xato: {e}"
                        )
                        stats["errors"] += 1

        logger.info(
            f"🏁 Yakunlandi. Topildi: {stats['found']}, Ko'chirildi: {stats['offloaded']}, Xatolar: {stats['errors']}"
        )
        return stats
