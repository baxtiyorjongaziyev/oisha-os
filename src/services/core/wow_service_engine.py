...
class WowServiceEngine:
    ...
    async def _audit_project_journey(self, project: Dict[str, Any]):
        ...
        pm_handle = AirtableSync.resolve_pm_handle(AirtableSync._get_field(fields, "manager"))
    ...
    async def _trigger_checkpoint(self, external_id: str, key: str, delay_hours: int, target_user_id: Optional[int], message: str, wow_action: str):
        checkpoint = await self.db.get_checkpoint(external_id, key)
        
        if checkpoint and checkpoint['status'] == 'Done':
            return
            
        created_at = checkpoint['created_at'] if checkpoint else datetime.datetime.now().isoformat()
        if not checkpoint:
        ...
        if target_user_id and self.bot_client:
            full_msg = f"{message}\n\n💡 <b>WOW MOMENT:</b>\n{wow_action}"
            try:
                await self.bot_client.send_message(target_user_id, full_msg, parse_mode='html')
                await self.db.mark_checkpoint_notified(external_id, key)
                logger.info(f"[WOW-ENGINE] Nudge sent to {target_user_id} for {key}")
            except Exception as e:
                logger.error(f"[WOW-ENGINE ERROR] Failed to nudge {target_user_id}: {e}")
