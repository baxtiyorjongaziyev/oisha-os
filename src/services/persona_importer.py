        ...
        try:
            ...
            for chat in chats_list:
                ...
                for msg in messages:
                    ...
                    if len(text) > 50 and any(kw in text.lower() for kw in ["branding", "agentlik", "shartnoma", "narx", "loyiha"]):
                        fact = f"OTMISH BILIMI ({chat_name}, {msg.get('date')}): {text[:500]}"
                        db.add_learned_fact(0, fact)
                        total_learned += 1
            
            logger.info(f"[IMPORT] Yakunlandi. {total_learned} ta muhim fakt Miya ga qoshildi.")
            return True
