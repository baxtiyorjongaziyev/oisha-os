    ...
    def _authenticate(self):
        ...
        if os.path.exists("token.pickle"):
            try:
                ...
                logger.info("[GCALENDAR OK] Shaxsiy Google akkaunti (Token) ishga tushdi.")
        ...
        if not creds:
            if os.path.exists(self.credentials_path):
                try:
                    ...
                    logger.error(f"[GCALENDAR] Service Account avtorizatsiyasida xato: {e}")
            else:
                logger.warning(f"[GCALENDAR] Avtorizatsiya fayli topilmadi.")
    ...
    def get_free_slots(self, date_str=None):
        ...
        try:
            ...
            for event in events:
                ...
                if 'T' in start:
                    busy_slots.append((datetime.datetime.fromisoformat(start).astimezone(tz), 
            ...
            if not free_slots and not date_str:
                next_day = (target_date + datetime.timedelta(days=1)).strftime("%Y-%m-%d")
            ...
            return [f"{date_formatted} tahriridagi: {slot}" for slot in free_slots] if free_slots else []
