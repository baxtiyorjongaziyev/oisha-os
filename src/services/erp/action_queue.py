...
class ActionQueue:
    ...
    async def wait_for_compensation(self, item: QueueItem, reason: str) -> None:
        now = get_local_now().isoformat()
        conn = await self.repo.db.get_connection()
        await conn.execute(
            """
            UPDATE erp_actions
            SET status = 'waiting_compensation',
                lease_owner = NULL,
                lease_expires_at = NULL,
                last_error = ?,
                updated_at = ?
        ...
        await self._finish_attempt(item, "waiting_compensation", {}, reason)
        await conn.commit()
