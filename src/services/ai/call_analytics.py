    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "avg_duration_min": round(
                self.total_duration / max(self.total_calls, 1) / 60, 1
            ),
...
class LostClientAnalysis:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "preventable_percent": round(
                self.preventable_count / max(self.total_lost, 1) * 100, 1
            ),
...
class CallAnalytics:
    ...
    def get_manager_dashboard(
        ...
        analyses = [
            a
            for a in self._filter_by_date(start_date, end_date)
        ...
        return {
            ...
            "recent_calls": [
                a.to_dict()
                for a in sorted(analyses, key=lambda x: x.analyzed_at, reverse=True)[
                    :10
                ]
            ],
        }
    ...
    def _get_daily_stats(
        ...
        for date_str in daily:
            day_analyses = [
                a for a in analyses if a.analyzed_at.strftime("%Y-%m-%d") == date_str
            ]
            if day_analyses:
                daily[date_str].average_score = sum(
                    a.overall_score for a in day_analyses
                ) / len(day_analyses)

        # Sanaga qarab sortlash
        return [daily[d].to_dict() for d in sorted(daily.keys())]

    def _get_manager_ratings(
        self, analyses: List[ConversationAnalysis]
    ) -> List[Dict[str, Any]]:
        """Manager reytinglari."""
        ...
        for manager_id, manager_analyses in by_manager.items():
            perf = ManagerPerformance(
                manager_id=manager_id,
                manager_name=(
                    manager_analyses[0].manager_name if manager_analyses else ""
                ),
            )

            perf.total_calls = len(manager_analyses)
            perf.average_score = sum(a.overall_score for a in manager_analyses) / len(
                manager_analyses
            )
            ...
            perf.conversion_rate = (
                sales / perf.total_calls * 100 if perf.total_calls > 0 else 0
            )

            # Taqsimot
            for a in manager_analyses:
                perf.score_distribution[a.category] = (
                    perf.score_distribution.get(a.category, 0) + 1
                )
    ...
    def _get_quality_distribution(
        ...
        return {"by_category": dict(categories), "by_score_range": score_ranges}

    def _get_outcome_analysis(
        self, analyses: List[ConversationAnalysis]
    ) -> Dict[str, Any]:
        """Natija tahlili."""
        outcomes = defaultdict(
            lambda: {"count": 0, "avg_score": 0.0, "total_duration": 0.0}
        )
        ...
        for outcome in outcomes:
            ...
            if count > 0:
                outcomes[outcome]["avg_score"] = round(
                    outcomes[outcome]["avg_score"] / count, 1
                )
                outcomes[outcome]["avg_duration_min"] = round(
                    outcomes[outcome]["total_duration"] / count / 60, 1
                )

        return dict(outcomes)

    def _get_objection_analysis(
        self, analyses: List[ConversationAnalysis]
    ) -> Dict[str, Any]:
        """E'tirozlar tahlili."""
        all_objections = []
        ...
        for obj, stats in objection_success.items():
            rate = (
                stats["converted"] / stats["total"] * 100 if stats["total"] > 0 else 0
            )
        ...
        return {
            ...
            "most_common": [
                {"objection": obj, "count": count} for obj, count in common_objections
            ],
            "by_success_rate": objection_rates[:10],
        }

    def _analyze_lost_clients(
        self, analyses: List[ConversationAnalysis]
    ) -> Dict[str, Any]:
        """Yo'qotilgan mijozlar tahlili."""
        ...
        # O'rtacha ball
        analysis.avg_score_before_lost = sum(
            a.overall_score for a in lost_analyses
        ) / len(lost_analyses)
        ...
        analysis.preventable_count = len(
            [a for a in lost_analyses if 60 <= a.overall_score <= 80]
        )
    ...
    def _generate_recommendations(
        ...
        lost_rate = (
            len([a for a in analyses if a.outcome == "lost"]) / len(analyses) * 100
        )
        if lost_rate > 30:
            recommendations.append(
        ...
        conversion = (
            len([a for a in analyses if a.outcome == "sale"]) / len(analyses) * 100
        )
        if conversion < 20:
            recommendations.append(
