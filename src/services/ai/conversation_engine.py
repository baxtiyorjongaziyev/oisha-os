...
class ConversationEngine:
    ...
    async def _save_analysis_to_db(
        self, analysis: ConversationAnalysis, call_record: CallRecord
    ):
                ...
                (
                        ...
                        [
                            (
                                ...
                                if hasattr(score, "to_dict")
                                    ...
                                    "improvement_tips": getattr(
                                        score, "improvement_tips", []
                                    ),
                                }
                            )
                    ...
                    json.dumps(
                        [
                            ...
                            for t in analysis.recommended_tasks
                        ],
                        ensure_ascii=False,
                    ),
    ...
    def get_dashboard_metrics(self, days: int = 7) -> DashboardMetrics:
        ...
        recent_analyses = [
            a for a in self.analyses.values() if start_date <= a.analyzed_at <= end_date
        ]
        ...
        metrics.avg_score_week = sum(a.overall_score for a in recent_analyses) / len(
            recent_analyses
        )
        ...
        if today_analyses:
            metrics.avg_score_today = sum(
                a.overall_score for a in today_analyses
            ) / len(today_analyses)
    ...
    def _generate_dashboard_recommendations(
        self, analyses: List[ConversationAnalysis], metrics: DashboardMetrics
    ) -> List[str]:
            ...
            comparisons.append(
                {
                    ...
                    "conversion_rate": round(
                        outcomes.get("sale", 0) / len(analyses) * 100, 1
                    ),
    ...
    def get_call_details(self, call_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "transcript": (
                call.transcript[:1000] if call.transcript else None
            ),  # Preview
        }

    def get_trend_analysis(
        self, metric: str = "score", days: int = 30
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i in range(days):
            ...
            if not analyses:
                trend.append({"date": date, "value": 0, "count": 0})
                continue
            ...
            trend.append(
                {"date": date, "value": round(value, 1), "count": len(analyses)}
            )
    ...
    async def process_amocrm_lead_calls(
        ...
        analyses = [a for a in self.analyses.values() if a.lead_id == lead_id]
    ...
    def get_skills_radar_data(
        self, manager_id: Optional[int] = None, days: int = 7
    ) -> Dict[str, Any]:
        """
