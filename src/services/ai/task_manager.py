...
class AITaskManager:
        ...
        if auto_create and self.amocrm:
            ...
            for task in tasks:
                ...
                if created:
                    ...
                    logger.info(f"[AI TASK] AmoCRM da vazifa yaratildi: {task['title']}")
                else:
                    task["created_in_crm"] = False
                    logger.warning(f"[AI TASK] AmoCRM da vazifa yaratilmadi: {task['title']}")
        ...
        if not analysis.next_steps:
            ...
            for i, step in enumerate(analysis.next_steps[:3], 1):
                ...
                tasks.append({
                    ...
                    "priority": self.PRIORITY_HIGH if i == 1 else self.PRIORITY_MEDIUM,
    ...
    async def _create_in_amocrm(
        ...
        try:
            ...
            complete_till = int((datetime.now() + timedelta(hours=due_hours)).timestamp())
