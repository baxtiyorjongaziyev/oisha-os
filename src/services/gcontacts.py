...
class GoogleContactsSync:
    ...
    def _authenticate(self):
        ...
        if os.path.exists("token.pickle"):
            try:
                ...
                logger.info("[GCONTACTS OK] Shaxsiy Google akkaunti (Token) ishga tushdi.")
        ...
        if not creds:
            if os.path.exists(self.credentials_path):
                try:
                    ...
                    logger.error(f"[GCONTACTS] Service Account avtorizatsiyasida xato: {e}")
            else:
                logger.warning(f"[GCONTACTS] Avtorizatsiya fayli topilmadi.")
    ...
    def get_or_create_group(self, group_name: str, create_if_missing: bool = True) -> str:
        ...
        try:
            ...
            if create_if_missing:
                ...
                new_group = self.service.contactGroups().create(body={
                    "contactGroup": {
                        "name": group_name
                    }
                }).execute()
                ...
                logger.warning(f"[GCONTACTS] Guruh topilmadi va yaratish taqiqlandi: {group_name}")
                return None
        except Exception as e:
    ...
    def create_contact(self, first_name, last_name="", phones=None, note="", group_name=None):
        ...
        try:
            ...
            if group_name:
                ...
                group_resource_name = self.get_or_create_group(group_name, create_if_missing=create_new)
            ...
            logger.info(f"[GCONTACTS OK] Kontakt yaratildi: {first_name} {last_name} ({len(phone_entries)} ta raqam)")
            return True
        except Exception as e:
