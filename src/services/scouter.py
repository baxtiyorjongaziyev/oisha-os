...
class Scouter:
    ...
    async def get_user_dosye(self, user_id):
        ...
        try:
            ...
            # 2. Common Chats
            common = await self.client(GetCommonChatsRequest(user_id=user_id, max_id=0, limit=100))
            for chat in common.chats:
                dosye["common_groups"].append({
                    "id": chat.id,
                    "title": chat.title
                })
            ...
            for group in dosye["common_groups"][:3]:
                ...
                async for msg in self.client.iter_messages(group_id, limit=20):
                    ...
                    if msg.sender_id == user_id:
                        dosye["messages_in_groups"].append(f"Mijoz ({group['title']}): {msg.text}")
                    ...
                    # Agar xabar Owner'dan bo'lsa va u mijozga REPLY bo'lsa (yoki yaqin muloqot)
                        ...
                        if msg.reply_to and msg.reply_to.reply_to_msg_id:
                            ...
                            dosye["messages_in_groups"].append(f"Siz (Guruh: {group['title']}): {msg.text}")
                        elif text := msg.text:
                            # Guruhdagi umumiy muloqotda Owner xabari (mijoz ismi/usernamesi zikr qilingan bo'lsa yanada yaxshi)
                            dosye["messages_in_groups"].append(f"Siz (Guruh: {group['title']}): {text}")
