...
class Scorer:
    ...
    def calculate_score(user_data, dosye=None):
        ...
        if user_data.get('username'): score += 5
        if user_data.get('phone'): score += 5
        ...
        expert_keywords = ['ceo', 'founder', 'asoschi', 'rahbar', 'manager', 'direktor', 'director', 'owner', 'tadbirkor']
        business_keywords = ['marketing', 'brending', 'branding', 'biznes', 'business', 'loyiha', 'project', 'invest']
        ...
        if mutual_count > 0:
            if mutual_count >= 5: score += 25
            elif mutual_count >= 3: score += 15
            else: score += 10

        # 4. Suhbat mazmuni (Mijozning guruhlardagi gaplari) (Max 40)
        messages = dosye.get('messages_in_groups', [])
        hot_keywords = ['narx', 'qancha', 'xizmat', 'shartnoma', 'loyiha', 'branding', 'logo', 'sayt', 'uchrashuv', 'zakaz', 'buyurtma']
        ...
        if msg_hits >= 3: score += 40
        elif msg_hits >= 1: score += 20
    ...
    @staticmethod
    def get_tier(score):
        if score >= 80: return "VIP (🔥 ISSIQ)"
        if score >= 50: return "Sifatli (ILIQ)"
        return "Oddiy (SOVUQ)"
