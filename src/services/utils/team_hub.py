...
class TeamHub:
    ...
    def identify_member(user_id, username="", text_context=""):
        ...
        if text_context:
            ...
            if any(w in low_text for w in ["dizayn", "logo", "figma"]):
                return "designer"
            if any(w in low_text for w in ["sotuv", "mijoz yopildi", "puls"]):
                return "closer"
            if any(w in low_text for w in ["proyekt", "deadline", "topshiriq"]):
                return "pm"
    ...
    def activate_ai_substitute(role):
        ...
        AgencyHR.hire_ai_agent(specialty)
    ...
    def get_daily_discipline_summary():
        ...
        with db.get_connection() as conn:
            ...
            # Hisobotlarni olish
            cursor.execute(
                "SELECT u.first_name, r.report_type, r.status FROM users u JOIN team_reports r ON u.user_id = r.user_id WHERE r.report_date = ?",
                (today,),
            )
            rows = cursor.fetchall()
            for name, r_type, status in rows:
                if r_type == "morning_plan":
                    summary["plans"].append(name)
                elif r_type == "evening_result":
                    summary["results"].append(name)
...
class StrategicAdvice:
    ...
    def get_missing_roles():
        ...
        return [
            {
                "role": "AI Brand Copywriter",
                "desc": "Brend hikoyasini dunyo darajasida yozish uchun.",
            },
            {
                "role": "UX/UI Strategist",
                "desc": "Brendni raqamli dunyoda yashashi uchun.",
            },
            {
                "role": "Data Scientist / Market Analyst",
                "desc": "Trendlarni oldindan bashorat qilish uchun.",
            },
            {
                "role": "Customer Success Manager",
                "desc": "Mijozni 'Ambassador' qilish jarayonini boshqarish uchun.",
            },
            {
                "role": "QA (Quality Assurance)",
                "desc": "Har bir dizayn va har bir harf BENUQSONligini tekshirish uchun.",
            },
        ]
