...
async def run_bulk_learning():
    ...
    logger.info(
        f"Jami {len(users)} ta foydalanuvchi topildi. O'rganishni boshlaymiz..."
    )
    ...
    for user_id in users:
        ...
        name = (
            user_info.get("first_name", f"User {user_id}")
            if user_info
            else f"User {user_id}"
        )
        ...
        for i in range(0, len(history), chunk_size):
            ...
            if added > 0:
                logger.info(
                    f"Foydalanuvchi ({user_id}) dan {added} ta yangi fakt o'rganildi."
                )
    ...
    logger.info(
        f"Ommaviy o'rganish yakunlandi! Jami {total_learned} ta yangi fakt bazaga qo'shildi."
    )
