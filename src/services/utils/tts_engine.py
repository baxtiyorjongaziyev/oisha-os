...
class TTSEngine:
    ...
    async def generate_audio(self, text, filename="response.mp3"):
        ...
        try:
            ...
            if config.TTS_PROVIDER == 'mohir' and hasattr(config, 'MOHIR_API_KEY') and config.MOHIR_API_KEY:
                try:
                    import requests
                    logger.info(f"[TTS] Mohir.ai orqali audio generatsiya qilinmoqda: {clean_text[:50]}...")
                    ...
                    response = requests.post("https://api.mohir.ai/api/v1/tts", headers=headers, json=json_data, timeout=15)
                    if response.status_code == 200:
                        with open(path, 'wb') as f:
                            f.write(response.content)
                        return path
                    else:
                        logger.error(f"[TTS ERROR] Mohir.ai status: {response.status_code}, Response: {response.text}")
            ...
            if config.TTS_PROVIDER == "elevenlabs" and config.ELEVENLABS_API_KEY:
                ...
                logger.info(f"[TTS] ElevenLabs orqali audio yaratilmoqda...")
                audio_gen = client.generate(
                    text=clean_text,
                    voice=config.ELEVENLABS_VOICE_ID if config.ELEVENLABS_VOICE_ID else "Bella",
            ...
            logger.info(f"[TTS] edge-tts orqali audio yaratilmoqda...")
            communicate = edge_tts.Communicate(clean_text, self.voice, rate="-10%", pitch="+15Hz")
