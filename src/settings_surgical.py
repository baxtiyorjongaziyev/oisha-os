...
class SurgicalSettings(BaseSettings):
    ...
    SURGICAL_MODE: bool = Field(
        default=False,
        description="Avtonom negotsiatsiya rejimi"
    )
    ...
    HUMAN_TAKEOVER_THRESHOLD: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="Inson nazoratiga o'tish chegarasi"
    )
    
    # Pricing defaults
    DEFAULT_SERVICE_PRICE_MIN: int = Field(
        default=1000,
        description="Eng arzon xizmat narxi ($)"
    )
    
    DEFAULT_SERVICE_PRICE_BASE: int = Field(
        default=5000,
        description="O'rtacha xizmat narxi ($)"
    )
    
    DEFAULT_SERVICE_PRICE_MAX: int = Field(
        default=25000,
        description="Eng qimmat xizmat narxi ($)"
    )
    ...
    CONTRACT_DEFAULT_TIMELINE_DAYS: int = Field(
        default=14,
        description="Standart shartnoma muddati (kun)"
    )
    
    # Automation settings
    STALE_LEAD_DAYS: int = Field(
        default=3,
        description="Qaysi kundan keyin lead 'stale' hisoblanadi"
    )
    
    FOLLOW_UP_DAYS: int = Field(
        default=2,
        description="Taklifdan keyin follow-up kunlari"
    )
    
    # Risk assessment weights
    RISK_WEIGHT_PRICE_PRESSURE: float = Field(
        default=0.3,
        description="Narx bosimi xavf vazni"
    )
    
    RISK_WEIGHT_COMPETITIVE: float = Field(
        default=0.4,
        description="Raqobat xavf vazni"
    )
    
    RISK_WEIGHT_NEGATIVE_SENTIMENT: float = Field(
        default=0.5,
        description="Salbiy kayfiyat xavf vazni"
    )
    
    # Notifications
    NOTIFY_ON_HIGH_VALUE_DEAL: bool = Field(
        default=True,
        description="Katta bitimlar haqida xabar berish"
    )
    
    HIGH_VALUE_THRESHOLD: int = Field(
        default=15000,
        description="Katta bitim chegarasi"
    )
    
    NOTIFY_ON_HUMAN_TAKEOVER: bool = Field(
        default=True,
        description="Inson nazoratiga o'tganda xabar berish"
    )
    
    # AI settings
    NEGOTIATION_AI_MODEL: str = Field(
        default="gemini-1.5-flash",
        description="Negotsiatsiya uchun AI model"
    )
    ...
    AI_TEMPERATURE: float = Field(
        ...
        ge=0.0,
        le=1.0,
        description="AI kreativlik darajasi"
    )
    
    AI_MAX_TOKENS: int = Field(
        default=300,
        description="AI javob maksimal uzunligi"
    )
    
    # Pipeline settings
    PIPELINE_AUTO_ADVANCE: bool = Field(
        default=True,
        description="Avtomatik bosqich o'tish"
    )
    
    PIPELINE_DAILY_REPORT: bool = Field(
        default=True,
        description="Kunlik hisobot yaratish"
    )
