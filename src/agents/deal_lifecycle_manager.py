...
class Deal:
    ...
    def _update_next_action(self):
        ...
        hours_map = {
            DealPriority.HOT: 4,
            DealPriority.WARM: 24,
            DealPriority.COLD: 72
        }
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "next_action": self.next_action_due.isoformat() if self.next_action_due else None
        }
...
class DealLifecycleManager:
        ...
        self.automation_rules = [
            {
                ...
                "condition": lambda d: d.days_in_stage() > 3 and d.stage == DealStage.NEW,
                "action": "send_revival_message",
                "priority": DealPriority.WARM
            },
            {
                "name": "Proposal Follow-up",
                "condition": lambda d: d.days_in_stage() > 2 and d.stage == DealStage.PROPOSAL,
                "action": "send_proposal_reminder",
                "priority": DealPriority.HOT
            },
            {
                "name": "Negotiation Push",
                "condition": lambda d: d.days_in_stage() > 5 and d.stage == DealStage.NEGOTIATION,
    ...
    def get_deal_by_user(self, user_id: str) -> Optional[Deal]:
        ...
        for deal in self.deals.values():
            if deal.user_id == user_id and deal.stage not in [
                DealStage.CLOSED_WON, DealStage.CLOSED_LOST, DealStage.NO_RESPONSE
            ]:
                return deal
        return None
    ...
    def register_stage_handler(
        self, 
        stage: DealStage, 
        handler: Callable[[Deal, Optional[DealStage]], Any]
    ):
    ...
    async def _trigger_handlers(
        ...
        deal: Deal, 
        stage: DealStage,
        old_stage: Optional[DealStage] = None
    ):
    ...
    def get_pipeline_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_deals": len([d for d in self.deals.values() 
                               if d.stage not in [DealStage.CLOSED_WON, DealStage.CLOSED_LOST]]),
            "stage_distribution": stage_counts,
            "total_pipeline_value": total_value,
            "overdue_actions": overdue_count,
            "hot_deals": len([d for d in self.deals.values() 
                            if d.priority == DealPriority.HOT and 
                            d.stage not in [DealStage.CLOSED_WON, DealStage.CLOSED_LOST]])
        }
