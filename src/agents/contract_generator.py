...
"""
        ...
        return {
            "branding": ContractTemplate(
                ...
                clauses=[
                    {"title": "Revision Policy", "text": "3 ta bepul revision, keyingilar 500,000 so'm"},
                    {"title": "Source Files", "text": "Source fayllar shartnoma tugagach beriladi"}
                ],
                variables=["CLIENT_NAME", "SERVICE_SCOPE", "TOTAL_PRICE", "TIMELINE", "START_DATE", "END_DATE"]
            ),
            "web": ContractTemplate(
                ...
                clauses=[
                    {"title": "Hosting", "text": "Hosting Mijoz tomonidan ta'minlanadi"},
                    {"title": "Domain", "text": "Domain Mijoz mulki"}
                ],
                variables=["CLIENT_NAME", "SERVICE_SCOPE", "TOTAL_PRICE", "TIMELINE", "SUPPORT_PRICE"]
            ),
            "marketing": ContractTemplate(
                ...
                clauses=[
                    ...
                    {"title": "Ad Spend", "text": "Reklama byudjeti Mijoz tomonidan qoplanadi"}
                ],
                variables=["CLIENT_NAME", "SERVICE_SCOPE", "TOTAL_PRICE", "TIMELINE", "KPI_TARGETS"]
            )
        }
    ...
    def generate_contract(
        ...
        return {
            ...
            "status": "draft",
            "requires_approval": deal_data.get("value", 0) > 10000  # 10k dan yuqori bo'lsa tasdiqlash kerak
        }
...
class RiskAssessor:
        ...
        for flag in negotiation_flags:
            if flag in self.risk_factors:
                ...
                if flag == "price_pressure":
                    recommendations.append("Qiymat asoslangan ROI hisobotini taqdim eting")
                elif flag == "competitive_pressure":
                    recommendations.append("Differentiation strategiyasini kuchaytiring")
                elif flag == "negative_sentiment":
                    recommendations.append("Human manager qo'llab-quvvatlashini talab qiling")
        
        # Value-based risks
        if deal_value > 20000:
            total_risk_score += 0.2
            risk_breakdown.append({
                "factor": "high_value_deal",
                "weight": 0.2,
                "level": "high"
            })
    ...
    def get_approval_requirements(self, risk_assessment: Dict) -> List[str]:
        ...
        if risk_assessment["score"] > 0.7:
            ...
            requirements.extend([
                "Manager tasdiqlashi",
                "Shartnoma shartlari ko'rib chiqilsin"
            ])
