...
logger = logging.getLogger("farmer_audit")

async def get_farmer_stages():
    ...
    if res.status_code == 200:
        ...
        for p in pipelines:
            name = p.get("name")
            p_id = p.get("id")
            if "Farmer" in name or "FARMER" in name.upper():
                logger.info(f"FOUND FARMER PIPELINE: {name} (ID: {p_id})")
