...
def main():
    ...
    print("Fetching leads that were in Sales Pipeline (10117998)...", flush=True)
    leads = []
    page = 1
    while True:
    ...
    print(f"Total leads found: {len(leads)}", flush=True)
    
    one_day_ago = int((datetime.now() - timedelta(days=1)).timestamp())
    recently_updated_leads = [l for l in leads if l['updated_at'] > one_day_ago and l['status_id'] in [143, 87609514]]
    
    print(f"Leads updated today (potential victims): {len(recently_updated_leads)}", flush=True)
    
    updates = []
    
    for idx, lead in enumerate(recently_updated_leads):
        lead_id = lead['id']
    ...
    print(f"\nTotal active leads to restore: {len(updates)}", flush=True)
