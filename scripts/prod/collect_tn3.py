...
async def collect_tn3():
    ...
    try:
        ...
        print("🔍 TEZ NATIJA 3 qidirilmoqda...")
        target = None
        async for dialog in client.iter_dialogs():
            if "tez natija 3" in dialog.name.lower():
                target = dialog
        ...
        async for user in client.iter_participants(target.id):
            ...
            last_name = f"{last} TN3 Gr" if last else "TN3 Gr"
            ...
            if phone:
                with_phone.append({
                    ...
                    'Notes': f"@{username}" if username else "TN3", 'Labels': 'TEZ NATIJA 3'
                })
                print(f"📱 {full} - {phone}")
            else:
        ...
        # CSV
        if with_phone:
            with open('tn3_with_phone.csv', 'w', newline='', encoding='utf-8') as f:
                w = csv.DictWriter(f, fieldnames=['Name', 'Given Name', 'Family Name', 'Phone 1 - Value', 'Phone 1 - Type', 'Notes', 'Labels'])
                w.writeheader()
                w.writerows(with_phone)
            print(f"\n📁 tn3_with_phone.csv: {len(with_phone)} ta")
        
        if without_phone:
            with open('tn3_no_phone.csv', 'w', newline='', encoding='utf-8') as f:
                w = csv.DictWriter(f, fieldnames=['Name', 'Given Name', 'Family Name', 'Notes', 'Labels'])
                w.writeheader()
                w.writerows(without_phone)
            print(f"📁 tn3_no_phone.csv: {len(without_phone)} ta")
        
        print(f"\n✅ TN3: {len(with_phone)} telefon, {len(without_phone)} telefonsiz")
...
if __name__ == "__main__":
    print("=" * 50)
    print("👸 OISHA: TEZ NATIJA 3")
    print("=" * 50)
    asyncio.run(collect_tn3())
