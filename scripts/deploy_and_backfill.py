...
host = "ubuntu@163.192.10.104"

files = [
    ("src/services/core/instagram/leadgen_delivery.py", "/home/ubuntu/oisha-os/src/services/core/instagram/leadgen_delivery.py"),
    ("src/services/core/instagram/leadgen_router.py", "/home/ubuntu/oisha-os/src/services/core/instagram/leadgen_router.py"),
    ("src/handlers/msg_pipeline/ai_reply.py", "/home/ubuntu/oisha-os/src/handlers/msg_pipeline/ai_reply.py"),
    ("src/bootstrap/orchestration/domain_agents.py", "/home/ubuntu/oisha-os/src/bootstrap/orchestration/domain_agents.py"),
    ("src/entrypoint/message_event.py", "/home/ubuntu/oisha-os/src/entrypoint/message_event.py"),
]

print("1. Uploading fixed files to Oracle VM...")
for local, remote in files:
    cmd = ["scp", "-i", ssh_key, "-o", "StrictHostKeyChecking=no", local, f"{host}:{remote}"]
    res = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
    if res.returncode != 0:
        print(f"FAILED {local}: {res.stderr}")
        sys.exit(1)
    print(f"Uploaded: {local} -> {remote}")

print("\n2. Executing today's lead backfill to Google Sheets and Telegram...")
backfill_code = '''
import asyncio
import os
import sqlite3
...
print(f"Found {len(rows)} leads to backfill...")
meta_ads = MetaAdsClient()

for rowid, leadgen_id, lead_id, destination in rows:
    print(f"Processing row {rowid}: leadgen_id={leadgen_id}, amo_id={lead_id}, dest={destination}")
    ...
    print(f"  -> Sheets append: {sheets_ok}")
...
cmd_backfill = [
    ...
    host,
    f"""/home/ubuntu/oisha-os/venv/bin/python3 -c '{backfill_code}'"""
]
res_backfill = subprocess.run(cmd_backfill, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=120)
print(res_backfill.stdout)
if res_backfill.stderr:
    print("BACKFILL STDERR:", res_backfill.stderr)

print("\n3. Restarting oisha-os.service on Oracle VM...")
cmd_restart = [
    "ssh", "-i", ssh_key, "-o", "StrictHostKeyChecking=no",
    host,
    "sudo systemctl restart oisha-os.service && sleep 2 && systemctl status oisha-os.service --no-pager"
]
res_restart = subprocess.run(cmd_restart, capture_output=True, text=True, encoding='utf-8', errors='replace', timeout=30)
print(res_restart.stdout)
if res_restart.stderr:
    print("RESTART STDERR:", res_restart.stderr)
