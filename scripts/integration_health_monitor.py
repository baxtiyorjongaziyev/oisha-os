...
def check_amocrm() -> Tuple[bool, str]:
    """AmoCRM OAuth token va API v4 holatini haqiqiy tekshiradi."""
    ...
    try:
        ...
        if not token and crm.token_data.get("refresh_token"):
            crm.refresh_token()
            token = crm.access_token

        if token:
            api_url = f"https://{subdomain}.amocrm.ru/api/v4/account"
            headers = {"Authorization": f"Bearer {token}"}
            resp = requests.get(api_url, headers=headers, timeout=DEFAULT_TIMEOUT)
            if resp.status_code == 200:
                acc_name = resp.json().get("name", subdomain)
                return True, f"AmoCRM: OK ('{acc_name}', status 200)"
            if resp.status_code == 401:
                if crm.refresh_token():
                    ...
                    if resp.status_code == 200:
                        acc_name = resp.json().get("name", subdomain)
                        return True, f"AmoCRM: OK (Refreshed, '{acc_name}')"
                return False, f"AmoCRM: Token muddati tugagan (API status {resp.status_code})"
            return False, f"AmoCRM: API Xatolik (status {resp.status_code})"

        url = f"https://{subdomain}.amocrm.ru"
        resp = requests.get(url, timeout=DEFAULT_TIMEOUT)
        if resp.status_code in (200, 301, 302, 401):
            return True, f"AmoCRM: OK (Tarmoq mavjud, status {resp.status_code})"
